...
addToBillBtn.addEventListener('click', function(){
    ...
    callTotalOneElem.innerHTML = textBill.callTotal().toFixed(2);
    // smsTotalOneElem.innerHTML = smsTotal.toFixed(2);
    smsTotalOneElem.innerHTML = textBill.smsTotal().toFixed(2);
    // var totalCost = callsTotal + smsTotal;
    // totalOneElem.innerHTML = totalCost.toFixed(2);
    totalOneElem.innerHTML = textBill.grandTotal().toFixed(2);



    if (parseInt(textBill.grandTotal()) >= 50){
        // adding the danger class will make the text red
        totalOneElem.classList.add("danger");
    }
    else if (parseInt(textBill.grandTotal()) >= 30){
